	...
	function init(){
		...
		var code=$("#codeId").val();
		var t=$("#tid").val();
		  $.post(localJsPreUrl+"/dm/coupon.php?code="+code+"&nnd=1&t="+t,{},
				...
				if(!data){
				   data=[];
			    	...
			    	for(var i=0;i<data.length;i++){
			    		...
			    		var aid=item.aid;
					var curl='http://go.hisprice.cn/vv/dm/couponClick.php?aid='+aid+"&couponAmount="+couponAmount+"&minLimitPrice="+minLimitPrice+"&t=";
	...
	function queryRecommend(){
		...
		var code=$("#codeId").val();
		var t=$("#tid").val();
		  $.post(localJsPreUrl+"/dm/couponRecomm.php?isRecommend=1&code="+code+"&t="+t,{},
  ...
  if(wWidth<400){
   isWap=1;
				...
				if(flag==3){ 
		    			aHref= "http://go.hisprice.cn/jd/m/myJdJump.php?f=vhis&action=jdJump&skuId="+itemId;
		    			
		    		}else{
		    		
				    //	aHref="/data/pub/jump.php?f=vhis&hasCoupon=1&iid="+itemId;	
		    	            aHref="http://go.hisprice.cn/vv/dm/goCorr.php?f=vhis&iid="+itemId;	
                               }
	...
	function queryHis(){
		...
		var code=$("#codeId").val();
		var t=$("#tid").val();
		var url=localJsPreUrl+"/dm/historynew.php?code="+code+"&t="+t+pram+adddata;
		addScriptTag(url);
		/*
		  $.post(,data,
